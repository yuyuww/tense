"""Plot helpers for unit trigram analysis outputs.

These functions are intentionally separate from the analysis code so the same
result dataframe can be explored with different x/y/group columns.
"""
...
def prepare_effect_plot_data(
    ...
    *,
    ...
    """Return numeric plotting data after optional count filtering."""
    needed = [x_col, y_col]
...
def plot_effect_scatter(
    ...
    *,
    ...
    """Flexible scatter plot for ratio, odds-ratio, log-odds, or delta columns."""
    import matplotlib.pyplot as plt

    data = prepare_effect_plot_data(
        df,
        x_col=x_col,
        y_col=y_col,
        group_col=group_col,
    ...
    if group_col:
        groups = data.groupby(group_col, dropna=False, observed=True)
...
def plot_binned_summary(
    ...
    *,
    ...
    """Plot a binned trend line for one effect column."""
    import matplotlib.pyplot as plt

    data = prepare_effect_plot_data(df, x_col=x_col, y_col=y_col, group_col=group_col)
    data["_bin"] = pd.cut(data[x_col], bins=bins)
    grouped_cols = [group_col, "_bin"] if group_col else ["_bin"]
    summary = (
        data.groupby(grouped_cols, observed=True)
